 ...
 * 商品カテゴリの一覧と編集フォームを描画します。
 */
?>
<h1>カテゴリ管理</h1>
<?php if ($saved): ?><div class="notice">保存しました。</div><?php endif; ?>
<section class="admin-panel">
    <form class="admin-form" method="post" action="<?= e(url('/admin/categories')) ?>">
        <?= csrf_field() ?>
        <input type="hidden" name="id" value="<?= e($edit['id'] ?? '') ?>">
        <label>カテゴリ名（日本語）<input name="name_ja" required value="<?= e($edit['name_ja'] ?? '') ?>"></label>
        <label>カテゴリ名（英語）<input name="name_en" value="<?= e($edit['name_en'] ?? '') ?>"></label>
...
<table class="admin-table">
    <thead><tr><th>ID</th><th>名前</th><th>slug</th><th>並び</th><th></th></tr></thead>
    <tbody>
